...
print(f"Total Months:{len(months)}", file=open("PyPoll.txt", "a"))
print(f"Average Change: {round(sum(change)/len(change),2)}", file=open("PyPoll.txt", "a"))
print(f"Greatest Increase in Profits: {months[monthincrease]} (${(str(increase))})", file=open("PyPoll.txt", "a"))
print(f"Greatest Decrease in Profits: {months[monthdecrease]} (${(str(decrease))})", file=open("PyPoll.txt", "a"))
